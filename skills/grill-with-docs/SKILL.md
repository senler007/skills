...
Read only the relevant authoritative design, glossary, architecture, ADR, active
Spec or tickets, and implementation context. Use `$grilling` to distinguish facts
that can be discovered from decisions that belong to the user.

Identify the likely existing authority for the topic before the first decision
question. This is provisional: do not create a new design boundary merely because
the interview topic has a convenient name.
...
2. Use `$project-documentation` to route the confirmed durable fact to exactly
   one authority.
3. Update an existing authoritative document when the current task authorizes
   documentation edits. Preserve the project's configured language, paths,
   casing, terminology, and style.
...
progress in the configured tracker, not in design documents.

## Handle document boundaries

Prefer the relevant section of an existing design document. If the confirmed
topic appears to need a new dedicated design document, explain the proposed reading
...
Summarize the confirmed design, updated documents, authoritative ownership, and
